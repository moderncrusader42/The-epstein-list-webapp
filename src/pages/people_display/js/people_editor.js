(function () {
  ...
  const normalizeReferenceTargetFromHref = (rawHref) => {
    ...
    if (path === "/sources-individual") {
      const sourceSlug = String(parsed.searchParams.get("slug") || "")
        .trim()
        .toLowerCase();
      if (sourceSlug) return `source:${sourceSlug}`;
    }
    if (path === "/theory-display") {
      const theorySlug = String(parsed.searchParams.get("slug") || "")
        .trim()
        .toLowerCase();
      if (theorySlug) return `theory:${theorySlug}`;
    }
    return href;
  };
