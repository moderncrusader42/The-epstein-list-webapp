...
def _normalize_reference_target_from_href(raw_href: str) -> str:
    ...
    if path == "/theory-display":
        query = parse_qs(parsed.query or "")
        slug_values = query.get("slug") or []
        if slug_values:
            theory_slug = str(slug_values[0] or "").strip().lower()
            if theory_slug:
                return f"theory:{theory_slug}"
    return href
...
def _build_citation_reference_entry(
    ...
    if theory_slug_match:
        ...
        return {
            ...
            "label": label,
            "preview": preview,
            "href": f"/theory-display/?slug={quote(theory_slug, safe='-')}",
            "external": False,
        }
    ...
    if theory_id_match:
        ...
        return {
            ...
            "label": label,
            "preview": preview,
            "href": f"/theory-display/?slug={quote(theory_slug, safe='-')}" if theory_slug else "",
            "external": False,
        }
