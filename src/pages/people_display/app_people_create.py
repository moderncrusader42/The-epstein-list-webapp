...
DEFAULT_CREATE_NAME = "New profile"
...
def _read_asset(path: Path) -> str:
    try:
        ...
        logger.warning("Missing people create asset at %s", path)
        return ""
...
def _header_people_create(request: gr.Request):
...
def _build_new_profile_seed() -> dict[str, str]:
    return {
        "slug": f"new-profile-{uuid4().hex[:8]}",
...
def _load_people_create_page(request: gr.Request):
    ...
    return (
        "<h2>New Card + Article Proposal</h2>",
        intro_update,
...
def _cancel_people_create_form():
    return _reset_create_editor_state()


def _submit_new_profile_proposal_with_refresh(
    proposal_note: str,
    proposal_name: str,
    proposal_bucket: str,
    proposal_tags: str,
...
def make_people_create_app() -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        with gr.Column(elem_id="people-shell"):
            with gr.Row(elem_id="people-title-row"):
                title_md = gr.HTML("<h2>New Card + Article Proposal</h2>", elem_id="people-title")
                gr.HTML(
                    "<a class='the-list-create-back-link' href='/the-list/'>Back to The List</a>",
        ...
        app.load(timed_page_load("/people-create", _header_people_create), outputs=[hdr])
        app.load(
            timed_page_load("/people-create", _load_people_create_page),
            outputs=[
                title_md,
                create_intro,
                card_proposal_help,
        ...
        submit_new_profile_btn.click(
            timed_page_load(
                "/people-create",
                _submit_new_profile_proposal_with_refresh,
                label="submit_new_profile_proposal_with_refresh",
            ),
            inputs=[
                proposal_note,
                card_proposal_name,
        ...
        cancel_new_profile_btn.click(
            timed_page_load("/people-create", _cancel_people_create_form, label="cancel_people_create_form"),
            outputs=[
                detail_html,
                current_slug,
                current_name,
