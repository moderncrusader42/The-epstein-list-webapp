(function () {
  ...
  const DROPDOWN_ID = "sources-tag-filter";
  const ALL_VALUE = "all";
  const PLACEHOLDER = "Filter by source tags";
  ...
  const stripNativeTick = (node) => {
    if (!node || node.dataset.sourcesTagTickStripped === "1") return;
    ...
    if (modified) {
      node.dataset.sourcesTagTickStripped = "1";
    }
  };

  const decorateOptions = (scope) => {
    optionNodes(scope).forEach((node) => {
      node.classList.add("sources-tag-option");
      ...
      node.classList.toggle("sources-tag-option--all", isAllValue(value) || isAllValue(label));
    });
  };
  ...
  const ensureScopedStyles = (scope) => {
    if (scope.querySelector("style[data-sources-tag-style]")) return;
    const style = document.createElement("style");
    style.dataset.sourcesTagStyle = "1";
    style.textContent = `
      .wrap .token,
      .token,
      .token-remove,
      ...
      .sources-tag-option.is-selected::before,
      .sources-tag-option[aria-selected="true"]::before,
      ...
      .sources-tag-option--all {
        font-weight: 700;
      }
      .sources-tag-option input[type="checkbox"],
      .sources-tag-option input[type="radio"],
      .sources-tag-option svg,
  ...
  const bindDropdown = () => {
    ...
    if (!host || host.dataset.sourcesTagDropdownBound === "1") return;
    ...
    if (!window._sourcesTagDropdownScrolls) {
      window._sourcesTagDropdownScrolls = new Map();
    }
    ...
    const getStoredScroll = () => {
      const fromGlobal = window._sourcesTagDropdownScrolls.get(DROPDOWN_ID);
      if (typeof fromGlobal === "number" && !Number.isNaN(fromGlobal)) return fromGlobal;
      return parseNumber(host.dataset.sourcesTagScroll || "0");
    };
    ...
    const rememberScrollPosition = (list, value) => {
      ...
      host.dataset.sourcesTagScroll = String(next);
      window._sourcesTagDropdownScrolls.set(DROPDOWN_ID, next);
    };
    ...
    const bindListScrollListeners = () => {
      optionLists().forEach((list) => {
        if (!list || list.dataset.sourcesTagScrollBound === "1") return;
        ...
        list.dataset.sourcesTagScrollBound = "1";
      });
    };

    const bindOptionScrollSnapshot = () => {
      optionNodes(scope).forEach((node) => {
        if (!node || node.dataset.sourcesTagSnapshotBound === "1") return;
        const snapshot = () => {
        ...
        node.dataset.sourcesTagSnapshotBound = "1";
      });
    };
    ...
    apply();
    host.dataset.sourcesTagDropdownBound = "1";
  };
