...
from src.pages.theories.core_theories import (
    ...
    _choice_values,
    _fetch_all_people as _fetch_all_theories,
    _filter_people_for_tag_selection as _filter_theories_for_tag_selection,
...
def _log_timing(event_name: str, start: float, **fields: object) -> None:
    ...
    if fields:
        ...
        timing_logger.info("theories.page.timing event=%s ms=%.2f %s", event_name, elapsed_ms, field_text)
        return
    timing_logger.info("theories.page.timing event=%s ms=%.2f", event_name, elapsed_ms)
...
def _read_asset(path: Path) -> str:
    ...
    try:
        ...
        logger.warning("Missing Theories asset at %s", path)
...
def _render_cards(people: Sequence[Dict[str, object]]) -> str:
    ...
    if not people:
        ...
        return '<div class="people-empty">No theories are available.</div>'

    cards: List[str] = []
    for row in people:
        name = html.escape(str(row.get("name") or "Unknown theory"))
        ...
        href = f"/theory-display/?slug={quote(slug, safe='-')}"
...
def _update_theory_cards_by_tag_filter(
    current_selection: Sequence[object] | None,
    previous_selection: Sequence[object] | None,
):
    total_start = time.perf_counter()
    step_start = time.perf_counter()
    people = _fetch_all_theories()
    _log_timing("update_tag_filter.fetch_theories", step_start, theories=len(people))
    step_start = time.perf_counter()
    ...
    step_start = time.perf_counter()
    filtered_people = _filter_theories_for_tag_selection(people, next_selection)
    _log_timing("update_tag_filter.filter_theories", step_start, filtered_theories=len(filtered_people))
    step_start = time.perf_counter()
    ...
    _log_timing(
        ...
        filtered_theories=len(filtered_people),
    )
    return dropdown_update, next_selection, cards_update


def _header_theories(request: gr.Request):
    start = time.perf_counter()
    header_html = render_header(path="/theories", request=request)
    _log_timing("header_theories.render_header", start, html_bytes=len(header_html))
    return header_html


def _load_theories_page(request: gr.Request):
    total_start = time.perf_counter()
    try:
        step_start = time.perf_counter()
        people = _fetch_all_theories()
        _log_timing("load_theories_page.fetch_theories", step_start, theories=len(people))
        step_start = time.perf_counter()
        selected_tags = _parse_tag_query_values(_query_param(request, "tag"))
        _log_timing("load_theories_page.parse_query_tags", step_start, selected_tags=len(selected_tags))
        step_start = time.perf_counter()
        ...
        _log_timing(
            "load_theories_page.build_tag_filter_update",
            step_start,
            selected=len(tag_filter_selection),
        )
        step_start = time.perf_counter()
        filtered_people = _filter_theories_for_tag_selection(people, tag_filter_selection)
        _log_timing("load_theories_page.filter_theories", step_start, filtered_theories=len(filtered_people))
        step_start = time.perf_counter()
        cards_html = _render_cards(filtered_people)
        _log_timing("load_theories_page.render_cards", step_start, html_bytes=len(cards_html))
        _log_timing(
            "load_theories_page.total",
            total_start,
            theories=len(people),
            selected=len(tag_filter_selection),
            filtered_theories=len(filtered_people),
        ...
        _log_timing("load_theories_page.error", total_start)
        logger.exception("Failed to load Theories page: %s", exc)
        return (
            "<h2>Theories</h2>",
            gr.update(visible=False),
            gr.update(choices=[(TAG_FILTER_ALL_OPTION, TAG_FILTER_ALL_OPTION)], value=[], interactive=True),
            [],
            gr.update(value='<div class="people-empty">Could not load theories.</div>', visible=True),
        )


def make_theories_app() -> gr.Blocks:
    ...
    with gr.Blocks(
        ...
        app.load(
            timed_page_load("/theories", _load_theories_page),
            outputs=[
                title_md,
                tag_filter_row,
                tag_filter,
        ...
        tag_filter.input(
            timed_page_load(
                "/theories",
                _update_theory_cards_by_tag_filter,
                label="update_theory_cards_by_tag_filter",
            ),
