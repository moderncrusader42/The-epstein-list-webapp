...
def _backfill_from_legacy_columns(session: Session) -> None:
    has_base_markdown = _column_exists(session, "people_change_proposals", "base_markdown")
    has_proposed_markdown = _column_exists(session, "people_change_proposals", "proposed_markdown")
    has_base_image_url = _column_exists(session, "people_change_proposals", "base_image_url")
    has_proposed_image_url = _column_exists(session, "people_change_proposals", "proposed_image_url")
...
def _backfill_from_legacy_diff_tables(session: Session) -> None:
    if _table_exists(session, "people_article_change_diffs"):
        session.execute(
            text(
                """
                UPDATE app.people_change_proposals p
                SET person_id = CASE
                    ...
                    END
                FROM app.people_article_change_diffs ad
    ...
    if not _table_exists(session, "people_card_change_diffs"):
        return
    ...
    card_rows = session.execute(
        text(
            ...
            FROM app.people_change_proposals p
            JOIN app.people_card_change_diffs cd
...
def ensure_people_diff_tables(session: Session) -> None:
    # Backward-compatible name: this now ensures unified proposal payload columns.
    session.execute(
        text(
            """
            ALTER TABLE app.people_change_proposals
            ADD COLUMN IF NOT EXISTS person_id BIGINT REFERENCES app.people(id) ON UPDATE CASCADE ON DELETE CASCADE
            """
        )
    )
    session.execute(
        text(
            """
            ALTER TABLE app.people_change_proposals
            ADD COLUMN IF NOT EXISTS base_payload TEXT
            """
        )
    )
    session.execute(
        text(
            """
            ALTER TABLE app.people_change_proposals
            ADD COLUMN IF NOT EXISTS proposed_payload TEXT
            """
        )
    )
    session.execute(
        text(
            ...
            WHERE p.person_slug = c.slug
              AND COALESCE(p.person_id, 0) <= 0
            """
        )
    )
    _backfill_from_legacy_columns(session)
    _backfill_from_legacy_diff_tables(session)
    session.execute(
        text(
            """
            UPDATE app.people_change_proposals
            SET base_payload = COALESCE(base_payload, ''),
...
def upsert_people_diff_payload(
    session: Session,
    *,
    proposal_id: int,
    person_id: int,
